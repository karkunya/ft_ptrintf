...
static int	apply_flags2(t_flags *flags, char *s_num, uintmax_t d, char ind)
{
	int	ret;
	int	sign;
	int	dot1;
...
static int	apply_flags3(t_flags *flags, char *s_num, uintmax_t d, char ind)
{
	int	ret;
	int	sign;
...
static int	apply_flags(t_flags *flags, uintmax_t d, char *s_num)
{
	int		ret;
	int		sign;
	char	ind;
...
int	treat_u(t_flags *flags)
{
	...
	d = va_arg(*(flags->argv), unsigned int);
	s_num = ft_itoa_base1(d);
	if (s_num == NULL)
		return (0);
